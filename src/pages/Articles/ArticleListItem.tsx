            ...
            {articleData.map((article: any) => (
                ...
                <div className=" p-4 flex flex-col leading-normal">
                  <div className="mb-8">
                    ...
                    <Link to={`articles/${article.id}`} className="text-blue-700 font-bold">read more...</Link>
                  </div>
                </div>
                <img className="h-48 lg:h-auto rg:w-48 flex-none bg-cover rounded-t lg:rounded-t-none lg:rounded-l text-center overflow-hidden" src={article.thumbnail} title="Thumbnail" style={{ float: 'right', maxHeight: '200px', maxWidth:'200px' }}
                />
              </Link>
            ))}
        </>
